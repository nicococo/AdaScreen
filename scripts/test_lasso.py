...
def remote_iteration(r, arguments, exms_to_load, directory):
    ...
    print res
    print res.shape
    output = 'results_{0}/intermediate/{1}'.format(arguments.dataset, arguments.screening_rule_set)
    if arguments.dataset=='Toy':
        output += '_(corr={0})'.format(arguments.corr)

    np.savez('{0}_run{1}_{2}'.format(output, r, props.getFname()), reps=arguments.reps, dataset=arguments.dataset, \
        legend=props.names, nexms=X.shape[0], x=x, results=[res], means=res, stds=np.zeros(res.shape), arguments=arguments)
...
def remote_save_result(results, props, x, arguments, exms_to_load, directory):
    ...
    np.savez('{0}{1}_{2}_{3}'.format(directory, \
        ...
        means=means, stds=stds, arguments=arguments, legend=props.names)
...
def main_method():
    ...
    parser.add_argument("-e","--experiment", help="experiment [0-5] (default 1)", default=6, type=int)
    ...
    parser.add_argument("-g","--geometric_mul", help="Multiplier for geometric path (default 0.9)", default=0.8, type=float)
    ...
    parser.add_argument("-c","--corr", help="Correlation coefficient for Toy dataset (default 0.6)", default=0.4, type=float)
