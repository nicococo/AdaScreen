...
from clustermap.conf import HEARTBEAT_FREQUENCY
from clustermap.data import zloads, zdumps
...
def get_memory_usage(pid, heart_pid):
    ...
    mem_total = float(process.memory_full_info().uss)
    for p in process.children():
        if p.is_running() and p.pid != heart_pid:
            try:
                mem_total += float(p.memory_full_info().uss)
